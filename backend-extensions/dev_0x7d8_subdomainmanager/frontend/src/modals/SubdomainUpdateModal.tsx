...
import updateSubdomain from '../api/updateSubdomain.ts';
import { subdomainEntrySchema, subdomainUpdateFormSchema } from '../lib/schemas.ts';
...
export default function SubdomainUpdateModal({ serverUuid, subdomain, onSuccess, ...modalProps }: Props) {
  ...
  const form = useForm<z.infer<typeof subdomainUpdateFormSchema>>({
    initialValues: { subdomain: '', domainUuid: '', allocationUuid: '' },
    validateInputOnBlur: true,
    validate: zod4Resolver(subdomainUpdateFormSchema),
  });
  ...
  useEffect(() => {
    ...
    form.setValues({
      ...
      domainUuid: subdomain.domain.uuid,
      allocationUuid: subdomain.allocation?.uuid ?? primaryAlloc?.uuid ?? '',
    });
  }, [modalProps.opened, subdomain.uuid, allocations]);

  const doSubmit = () => {
    setLoading(true);
    updateSubdomain(serverUuid, subdomain.uuid, form.values.allocationUuid)
      .then(() => {
  ...
  return (
    <Modal title={tExt('pages.server.subdomains.modal.update.title', {})} {...modalProps}>
