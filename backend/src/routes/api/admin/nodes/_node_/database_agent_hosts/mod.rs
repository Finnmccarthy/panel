...
mod get {
    ...
    struct Response {
        ...
        database_agent_hosts:
            Pagination<shared::models::node_database_agent_host::AdminApiNodeDatabaseAgentHost>,
    }
    ...
    pub async fn route(
        ...
        let database_agent_hosts = NodeDatabaseAgentHost::by_node_uuid_with_pagination(
            &state.database,
            node.uuid,
            params.page,
            params.per_page,
...
mod post {
    ...
    use shared::{
        ...
        models::{
            ...
            node::GetNode,
            node_database_agent_host::{CreateNodeDatabaseAgentHostOptions, NodeDatabaseAgentHost},
    ...
    pub async fn route(
        ...
        let options = CreateNodeDatabaseAgentHostOptions {
            node_uuid: node.uuid,
            database_agent_host_uuid: data.database_agent_host_uuid,
        };
        match NodeDatabaseAgentHost::create(&state, options).await {
            Ok(_) => {}
            Err(err) if err.is_unique_violation() => {
                return ApiResponse::error("database agent host already exists on this node")
        ...
        activity_logger
            .log(
                "node:database-agent-host.create",
                serde_json::json!({
                    "node_uuid": node.uuid,
                    "database_agent_host_uuid": data.database_agent_host_uuid,
                }),
