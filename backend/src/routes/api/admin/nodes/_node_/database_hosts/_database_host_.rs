...
mod delete {
    ...
    pub async fn route(
        ...
        Path((_node, database_host)): Path<(uuid::Uuid, uuid::Uuid)>,
    ) -> ApiResponseResult {
        permissions.has_admin_permission("nodes.database-hosts")?;

        let node_database_host = match NodeDatabaseHost::by_node_uuid_database_host_uuid(
            &state.database,
            node.uuid,
            database_host,
        )
        ...
        node_database_host.delete(&state, ()).await?;

        activity_logger
            .log(
                "node:database-host.delete",
                serde_json::json!({
                    "node_uuid": node.uuid,
                    "database_host_uuid": node_database_host.database_host.uuid,
                }),
            )
            .await;
